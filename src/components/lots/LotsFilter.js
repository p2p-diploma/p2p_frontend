...
export default function LotsFilter() {
    const [filter, setFilter] = useState({
        currencyType: 'eth', sellType: 'sell', paymentType: 'all', email: '', showPersonal: false
    });
    ...
    const onShowPersonalLots = e => {
        e.preventDefault();
        setFilter(prevState => ({
            ...prevState,
            showPersonal: !prevState.showPersonal
        }));
    }
            ...
            <div className="col-md-2">
                {Cookies.get('jwt-access') && Cookies.get('jwt-refresh') &&
                 <div class="form-check">
                    <input onClick={onShowPersonalLots} className="form-check-input" type="checkbox" value="show" id="showMy"
                     defaultChecked={filter.showPersonal} />
                    <label className="form-check-label" for="showMy">
                        Show my lots
                    </label>
                </div>}
            </div>
